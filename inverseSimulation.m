...
% out = y(1:size(y, 1)/2, :);
out = y;
out = out(info.pairs.NN==2 & info.pairs.WL == 2, :);
A_seg = A(info.pairs.NN(info.pairs.WL == 2) ==2, :);
...
%     end
% end
[filtered, D] = lowpass(data_wave_1', 0.1, 11, StopbandAttenuation=60, Steepness=.5, ImpulseResponse="fir");
filtered = filtered';
